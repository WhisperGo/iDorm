                                        ...
                                        @if(auth()->user()->role->role_name != 'Resident')
                                            <td class="text-center">
                                                <div class="d-flex align-items-center justify-content-center list-user-action">
                                                    ...
                                                    @if(auth()->user()->role->role_name == 'Manager')
                                                        <a class="btn btn-sm btn-icon btn-soft-primary" data-bs-toggle="tooltip" data-bs-placement="top" title="Edit Profile" href="{{ route('admin.profile.edit', $res->id) }}">
                                                    ...
                                                    @else
                                                        {{-- LOGIC ADMIN --}}
                                                        @php
                                                            $myFacilityId = auth()->user()->adminDetails->facility_id ?? 0;
                                                            $localSuspend = $res->activeSuspensions->firstWhere('facility_id', $myFacilityId);
                                                        @endphp

                                                        @if($localSuspend)
                                                            {{-- Tombol Hapus/Batalkan (Soft Danger Button) --}}
                                                            <form action="{{ route('suspensions.destroy', $localSuspend->id) }}" method="POST" onsubmit="return confirm('Cabut hukuman fasilitas untuk user ini?')">
                                                                @csrf @method('DELETE')
                                                                <button type="submit" class="btn btn-sm btn-icon btn-soft-danger" data-bs-toggle="tooltip" data-bs-placement="top">
                                                                    <span class="btn-inner">
